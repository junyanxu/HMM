...
data["Return"] = data["Adjusted Close"].diff(futureReturnPeriod).shift(-futureReturnPeriod)/data["Adjusted Close"]
data["10R"] = pandas.rolling_apply(data['Adjusted Close'], 10, getTrendR)
data["20R"] = pandas.rolling_apply(data['Adjusted Close'], 20, getTrendR)
data["30R"] = pandas.rolling_apply(data['Adjusted Close'], 30, getTrendR)
data = data.dropna()
...
for i in range(model.n_components):
    ...
    if(all(model.means_[i]>0.2)):
        up_component = i
    if model.means_[i][0] < -0.4 and model.means_[i][1] <-0.6 and model.means_[i][2] < -0.6:
        rev_component = i
    if model.means_[i][0]< -0.4 and model.means_[i][-1] > 0:
        down_component = i
...
res = pandas.DataFrame(
    {"Cum PNL":(data["Return"]*position).cumsum(),
     "SP500": data["Adjusted Close"],
     "Position":position*0.3
    })

res.plot(ax=axe, secondary_y="SP500")
